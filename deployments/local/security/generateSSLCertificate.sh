...
SCRIPT=generateSSLCertificate.sh
KEYSTORE=$1
VALIDITY=$2
...
main()
{
    ...
    CMD_RUN="keytool -keystore $DEFAULT_KEYSTORE_DIR/$KEYSTORE -alias localhost -validity $VALIDITY -genkey -keyalg RSA -destkeystoretype pkcs12"
    echo ${cyn}Generating SSL certificate with:${end} ${grn}$CMD_RUN${end}
    $CMD_RUN
}
