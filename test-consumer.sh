...
main()
{
    ...
    if [ $PLATFORM_OPTION == "2" ]; then
        CMD_RUN="$KAFKA_HOME/bin/kafka-console-consumer.sh --bootstrap-server $BOOTSTRAP_SERVER --consumer.config $KAFKA_HOME/config/consumer.properties --topic $KAFKA_TOPIC --from-beginning"
    fi
